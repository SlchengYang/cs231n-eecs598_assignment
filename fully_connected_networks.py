...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        flat_x = flat_x.to(w.dtype)
        out = flat_x.matmul(w) #(N, M)
        out += b.reshape(1,b.shape[0])
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        ##################################################
        # Replace "pass" statement with your code
        db = (dout.sum(dim = 0)).t() #M,1
        flat_x = x.flatten(start_dim = 1) #N*D
        flat_x = flat_x.to(w.dtype)
        dw = (flat_x.t()).matmul(dout) #D*M
        ls = x.shape
        dx = dout.matmul(w.t()) #N*D
        dx = dx.reshape(ls)
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        self.params['W1'] = torch.normal(0,weight_scale,size = [input_dim,hidden_dim])  #(D, M)
        self.params['b1'] = torch.zeros(hidden_dim) #(M,)
        self.params['W2'] = torch.normal(0,weight_scale,size = [hidden_dim,num_classes])  #(D, M)
        self.params['b2'] = torch.zeros(num_classes) #(M,)
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        dh, grads['W2'], grads['b2'] = Linear.backward(ds, cache_2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        h_dims = hidden_dims.copy()
        h_dims.insert(0,input_dim)
        h_dims.append(10)
        for i in range(1,len(h_dims)):
            self.params['W%d' %(i)] = torch.normal(0,weight_scale, size = [h_dims[i-1],h_dims[i]])
            self.params['b%d' %(i)] = torch.zeros(h_dims[i])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model,data_dict,optim_config = {'learning_rate':1},lr_decay = 0.5)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
