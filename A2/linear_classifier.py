...
def svm_loss_naive(
    ...
    for i in range(num_train):  
        scores = W.t().mv(X[i])    #第i个example各项得分的向量
        correct_class_score = scores[y[i]]  #正确项得分
...
def svm_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    examples = X.shape[0]
    scores = torch.matmul(W.t(), X.t()) # labels*examples 的矩阵
    correct_scores = scores[y,range(examples)] # 各个example的正确项得分
    margins = torch.maximum(torch.zeros_like(scores), scores-correct_scores+1)# 各项的loss
    margins[y,range(examples)] = 0 #剔除正确项得分
    loss = torch.sum(margins)/examples + reg*torch.sum(W*W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    #############################################################################
    # Replace "pass" statement with your code
    margins[margins>0] = 1 #标记算入损失函数系数的部分（不包括正确的）
    margins[y,torch.arange(examples)] = -torch.sum(margins,0) #把正确项填入对应值
    dW = margins.matmul(X)
    dW = dW.t()
    dW /= examples
    dW += 2 * reg * W 
...
def train_linear_classifier(
    ...
    if W is None:
        # lazily initialize W
        num_classes = torch.max(y) + 1 #取出最大的标签，y是全体标签集
    ...
    for it in range(num_iters):
        ...
        v = rho*v + (1-rho)*grad
        gs = k*gs + (1-k)*(grad**2)
        W -= (learning_rate*v)/(torch.sqrt(gs)+b)
        # M方法
        #v = rho*v + grad
        #W -= learning_rate*v
        #########################################################################
...
def test_one_param_set(
    ...
    train_acc = (cls.predict(X_train) == y_train).sum().item()/y_train.shape[0]
    val_acc = (cls.predict(X_val)==y_val).sum().item()/y_val.shape[0]
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    scores = X.matmul(W) # 行：examples，列：class
    for i in  range(scores.shape[0]):
        # 稳定性：-log C
        stable_const = scores[i].max()
        correct_score = scores[i][y[i]]
        up = torch.exp(correct_score-stable_const)
        #up = torch.exp(correct_score)
        down = 0.0
        for j in range(scores.shape[1]):#loss
            down += torch.exp(scores[i,j]-stable_const)
        p_correct = up/down
        loss -= torch.log(p_correct)


        for j in range(scores.shape[1]):#gradient
            p_j = torch.exp(scores[i, j] - stable_const) / down  
            if j == y[i]:
                dW[:, j] -= X[i] * (1 - p_j)  
            else:
                dW[:, j] += X[i] * p_j 
    ...
    loss = loss/scores.shape[0] + reg * torch.sum(W * W)
    dW /= scores.shape[0]
    dW += reg*2*W
    #############################################################################
    #                          END OF YOUR CODE                                 #
...
def softmax_loss_vectorized(
    ...
    scores = scores - scores.max(-1, True).values# 保持原有维度，数值形成的张量，stable
    softmax = scores - scores.logsumexp(-1, True)# scores.logsumexp(-1, True)分母部分一步到位
    loss = -softmax[range(num_train), y].mean() # 只取正确class的结果，即loss，求平均
    loss += reg * torch.sum(W * W)

    prob = softmax.exp() # (N, C) #每个ele取指数，即概率，prob：n*c， 
    mask = prob - torch.nn.functional.one_hot(y, prob.shape[1]) #独热编码，正确标签的概率减1
...
def softmax_get_search_params():
    ...
    regularization_strengths = [1, 1e-1, 1e-2, 1e-3, 1e-4]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
